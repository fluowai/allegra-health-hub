...
export function Hero() {
  return (
    <section id="home" className="relative overflow-hidden" style={{ backgroundColor: "#fcfbf8" }}>
      {/* Decorative organic shapes on the left edge */}
      <svg
      ...
      <div className="container-allegra grid gap-10 py-14 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] lg:gap-14 lg:py-20">
        <div className="relative z-10 flex flex-col justify-center">
          <h1 className="font-display text-[42px] font-extrabold leading-[1.05] text-brand-blue-dark sm:text-5xl md:text-[56px]">
            Cuidamos da saúde
            <br /> de quem você
            <br />
            <span className="text-brand-orange">mais ama.</span>
          </h1>
          <p className="mt-6 max-w-md font-sans text-base text-brand-steel md:text-[17px]">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <a
              ...
              className="inline-flex items-center gap-2 rounded-full bg-brand-green px-6 py-3.5 font-ui text-[13px] font-semibold uppercase tracking-wide text-white shadow-xl shadow-brand-green/30 transition-transform hover:-translate-y-0.5"
            >
            ...
            <a
              href="#especialidades"
              className="inline-flex items-center gap-2 rounded-full border-2 border-brand-blue bg-white px-6 py-3.5 font-ui text-[13px] font-semibold uppercase tracking-wide text-brand-blue transition-colors hover:bg-brand-blue hover:text-white"
            >
      ...
      <svg className="block w-full text-white" viewBox="0 0 1440 60" preserveAspectRatio="none" aria-hidden>
        <path fill="currentColor" d="M0,30 C240,70 480,0 720,20 C960,40 1200,60 1440,20 L1440,60 L0,60 Z" />
      </svg>
    </section>
  );
}
